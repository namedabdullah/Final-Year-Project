...
| `verification.py` | ~330 | `verify_question()` → Claude Sonnet grounded verifier. The LLM only *measures* (`answerable_from_context`, `actual_*`); the claimed-vs-actual **match booleans are computed deterministically** (reasoning by accepted-set — hard = causal/analytical, + inferential only if multi-piece; complexity by floor — hard ≥3; see `diagnostics`). OpenAI fallback, then conservative mock. **Locked prompt — leave untouched.** |
