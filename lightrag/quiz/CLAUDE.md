...
`diagnostics`, `storage` are leaves; `seeds`/`retrieval`/`generation`/`verification`
...
| `verification.py` | ~320 | `verify_question()` → Claude Sonnet grounded verifier (answerability + claimed-vs-actual complexity/reasoning match). OpenAI fallback, then conservative mock. |
...
diagnostics), and optional `verification`. Top level adds `file_contributions`, `diversity`,
and `warnings`.
