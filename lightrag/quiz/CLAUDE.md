...
| `verification.py` | ~330 | `verify_question()` → Claude Sonnet grounded verifier. The LLM only *measures* (`answerable_from_context`, `actual_*`); the claimed-vs-actual **match booleans are computed deterministically** (reasoning by depth-tier, complexity by floor — see `diagnostics`). OpenAI fallback, then conservative mock. **Locked prompt — leave untouched.** |
