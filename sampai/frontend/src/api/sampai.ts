...
export const classroomApi = {
  ...
  leave: (id: number): Promise<void> => api.post(`${P}/classrooms/${id}/leave`).then(() => undefined),
...
/** Stream a grounded answer over SSE. Calls onToken for each chunk; resolves on done. */
export async function streamChat(
  ...
  const res = await fetch(`${API_BASE}/api/sampai/chat/files/${fileId}/ask`, {
    method: 'POST',
...
export const groupChatApi = {
  ...
  eligible: (fileId: number, groupChatId?: number): Promise<UserSummary[]> =>
    api.get(`${G}/files/${fileId}/eligible-invitees`, { params: { group_chat_id: groupChatId } }).then((r) => r.data),
  invite: (
    fileId: number,
    userIds: number[],
    groupChatId?: number,
  ): Promise<{ group_chat_id: number; invites: GroupInvite[] }> =>
    api.post(`${G}/files/${fileId}/invite`, { user_ids: userIds, group_chat_id: groupChatId ?? null }).then((r) => r.data),
  pendingInvites: (): Promise<GroupInvite[]> => api.get(`${G}/invites/pending`).then((r) => r.data),
  accept: (inviteId: number): Promise<GroupChat> => api.post(`${G}/invites/${inviteId}/accept`).then((r) => r.data),
  reject: (inviteId: number): Promise<GroupInvite> => api.post(`${G}/invites/${inviteId}/reject`).then((r) => r.data),
  cancel: (inviteId: number): Promise<GroupInvite> => api.post(`${G}/invites/${inviteId}/cancel`).then((r) => r.data),
}
...
/** Build an authenticated WebSocket URL for a SAMpai WS path (e.g. "/api/sampai/group-chat/ws/user"). */
export function wsUrl(path: string): string {
  ...
  return `${origin}${path}${sep}token=${encodeURIComponent(token)}`
}
...
export interface FolderQuizDetail {
  ...
  score: number | null // aggregate 0–1
  correct_count: number | null
  total_count: number // questions actually generated
  graded_count: number
...
export const folderQuizApi = {
  ...
  get: (quizId: number): Promise<FolderQuizDetail> =>
    api.get(`${FQ}/${quizId}`).then((r) => r.data),
  submitQuestion: (
    quizId: number,
    questionId: string,
    userAnswer: string,
    ...
    api
      .post(`${FQ}/${quizId}/questions/${questionId}/submit`, { user_answer: userAnswer })
      .then((r) => r.data),
