...
import { groupChatApi, wsUrl, type GroupInvite, type WsEvent } from '@/api/sampai'
...
export function connectUserSocket(): void {
  ...
  // Seed pending invites so the bell is correct even before any live event.
  groupChatApi
    .pendingInvites()
  ...
  const open = () => {
    ...
    sock.onmessage = (ev) => {
      try {
        handle(JSON.parse(ev.data) as WsEvent)
      } catch {
...
function handle(e: WsEvent): void {
  ...
  switch (e.type) {
    case 'invite_new':
      ...
      toast.message(`${e.invite.inviter.username} invited you to a group chat`, { id: `invite-${e.invite.id}` })
      break
    case 'invite_cancelled':
      st.removeInvite(e.invite_id)
      toast.dismiss(`invite-${e.invite_id}`) // cancel dismisses the toast
      break
    ...
    case 'thread_unread_bump':
      ...
      toast.message('You were mentioned in a discussion')
      break
    case 'announcement_new':
      ...
      toast.message(`${e.author} posted an announcement`)
      break
    case 'comment_new':
      useRealtime.setState((s) => ({
        announcements: [
      ...
      toast.message(`${e.author} commented on your announcement`)
      break
    default:
      break
  }
