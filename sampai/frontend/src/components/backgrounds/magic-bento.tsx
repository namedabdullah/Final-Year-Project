    ...
    <>
      <style>{`
        ...
        .card--border-glow:hover {
          box-shadow: 0 6px 28px rgba(2, 6, 23, 0.15), 0 0 36px rgba(${glowColor}, 0.18);
        }
        .particle::before { content: ''; position: absolute; top: -2px; left: -2px; right: -2px; bottom: -2px;
          background: rgba(${glowColor}, 0.16); border-radius: 50%; z-index: -1; }
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-2">
          {cards.map((card, index) => {
            const baseClassName =
              `card flex flex-col justify-between relative aspect-[4/3] min-h-[200px] w-full max-w-full p-5 rounded-[20px] ` +
              `border border-solid overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-0.5 ` +
              `hover:shadow-[0_8px_25px_rgba(0,0,0,0.12)] ${enableBorderGlow ? "card--border-glow" : ""} ` +
              `bg-card border-border text-card-foreground`

            const cardStyle: CSSProperties = {
              backgroundColor: cardBg,
              borderColor: borderCol as string,
              ["--glow-x" as string]: "50%",
              ["--glow-y" as string]: "50%",
              ["--glow-intensity" as string]: "0",
              ["--glow-radius" as string]: "220px",
            }
            ...
            return enableStars ? (
              ...
              >
                <div className="card__header flex items-center justify-between gap-3 relative">
                  {Icon ? (
                    <div
                      ...
                      style={{
                        background: `radial-gradient(circle at 30% 30%, rgba(${glowColor},0.35) 0%, rgba(${glowColor},0.12) 55%, transparent 65%)`,
                ...
                <div className="card__content flex flex-col relative text-muted-foreground">
                  <h3 className={`card__title font-semibold text-xl m-0 mb-1 ${textAutoHide ? "text-clamp-1" : ""} text-foreground`}>
                    {card.title}
                  </h3>
                  <p className={`card__description text-[13px] leading-5 ${textAutoHide ? "text-clamp-2" : ""}`}>
                    {card.description}
                  </p>
                  {Icon && card.label ? (
                    <span
                      className="mt-3 inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] text-foreground"
                      style={{
