...
// Ported from sampai/frontend/src/hooks/use-group-chat-socket.ts.
// RN differences: crypto.randomUUID -> expo-crypto; WebSocket is a global.
...
export type TypingState = { userId: number; username: string; isTyping: boolean };
...
function mergeMessages(existing: GroupMessage[], incoming: GroupMessage[]): GroupMessage[] {
  ...
  for (const m of incoming) {
    if (m.client_msg_id) {
      ...
      if (map.has(tempKey)) map.delete(tempKey);
    }
    map.set(m.id, m);
  }
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'REPLACE_TEMP': {
      const withoutTemp = state.messages.filter((m) => m.id !== `temp:${action.clientMsgId}`);
    ...
    case 'DISCARD_MESSAGE':
      return {
        ...
        messages: state.messages.map((m) =>
          m.id === action.messageId ? { ...m, is_discarded: true, discard_reason: action.reason } : m,
        ),
      };
    ...
    case 'SET_READ_SEQ':
      return { ...state, readSeqs: { ...state.readSeqs, [action.userId]: action.lastSeq } };
    case 'SET_CONNECTED':
...
export function useGroupChatSocket(groupChatId: number | null) {
  ...
  const send = useCallback(async (content: string, replyToId?: number): Promise<GroupMessage> => {
